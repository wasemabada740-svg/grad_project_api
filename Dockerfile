...
RUN pip install --no-cache-dir "setuptools<82.0.0" && pip install --no-cache-dir -r requirements.txt

# Verify that pkg_resources is installed and accessible
RUN python -c "import pkg_resources; print('pkg_resources imported successfully from:', pkg_resources.__file__)"
